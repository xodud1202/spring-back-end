...
- `trimToNull`, `isBlank`, `safeValue`, 필수값 검증처럼 도메인 독립적으로 반복되는 메서드는 서비스마다 복제하지 말고 `common` 성격의 공통 파일로 승격한다.
- 서비스 간에 헬퍼 메서드를 우회 호출하는 방식으로 재사용하지 않는다. 공통화가 필요하면 공통 유틸 또는 공통 지원 클래스로 정리한다.
- `admin`, `bo`, `work` 등 엔드포인트 이름만 다르고 실제 동작이 같으면 서비스 메서드, 매퍼 메서드, SQL을 이름만 바꿔 복제하지 않는다. 실제 차이가 생길 때만 분리한다.
